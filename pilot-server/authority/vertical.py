...
vertical = Blueprint('vertical', __name__)


# 登录接口
@vertical.route('/api/auth/l3/login',methods=['POST'])
...
# 获取权限信息
@vertical.route('/api/auth/l3/getUserinfo',methods=['GET'])
...
# 垂直越权
@vertical.route('/api/auth/l3/modifyUserInfo',methods=['POST'])
@responses.check_header(['Authorization'])
...
def modifyUserInfo():
    ...
    if verify_token(token,account):
        ...
        if int(auth1) > int(auth2) or account == username:
...
SECRET_KEY = "PILOT/vertical_t0ken"
